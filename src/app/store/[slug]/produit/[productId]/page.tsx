...
export default async function PublicStoreProductPage({ params }: { params: Promise<{ slug: string; productId: string }> }) {
  ...
  const { data: product } = await supabase.from("products").select("id,name,note,description,price,image_url,is_available,is_featured").eq("id", productId).eq("store_id", store.id).eq("is_available", true).maybeSingle();
  if (!product) notFound();
  ...
  return <div style={themeCssVariables(tokens)}><StoreProductPage product={toProductView({...product,product_media:media??[]})} storeName={store.name} logoUrl={store.logo_url} whatsapp={store.whatsapp ?? ""} slug={store.slug} /></div>;
}