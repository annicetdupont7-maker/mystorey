...
export function SettingsForm({storeId,name,description,whatsapp}:{storeId:string;name:string;description:string;whatsapp:string}){
  ...
  return <form className="settings-form" action={action}>
    ...
    <label className="field"><span>Numéro WhatsApp</span><input name="whatsapp" defaultValue={whatsapp} placeholder="+237 6 90 00 00 00"/>{state.fieldErrors?.whatsapp&&<small>{state.fieldErrors.whatsapp[0]}</small>}<small className="field-hint">Format international : c’est sur ce numéro que vos clients enverront leurs commandes.</small></label>
